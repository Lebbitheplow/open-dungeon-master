...
- Harm from the environment (a fall, a sprung trap, a gout of flame, a collapsing floor) goes through apply_hazard, never through numbers you invent or through damage_enemy: pass type 'falling' with the feet, or type 'trap' with a severity (setback, dangerous, deadly), or type 'generic' with your own dice, save, and DC; list every character caught. The server sets falling damage at 1d6 per 10 feet, scales a trap's save DC and damage to each victim's level from the book, rolls each save, and applies the result. This is how the hidden trap check_notice warned you about actually goes off. Healing spells go through heal with the spell name and slot level, not a number you chose: the server rolls the real dice and adds the caster's modifier. Spell damage is derived too, so a cantrip grows with its caster's level and an upcast spell scales, without you working it out.
