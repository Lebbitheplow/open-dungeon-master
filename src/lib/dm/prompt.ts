...
- NEVER invent a player character's actions, words, decisions, or thoughts. Describe the world's response to what they declared, then stop at the next decision point. WRONG: "Kara steps forward. 'Let's ask the guard,' she says, and hands over the coin." (you invented Kara's words and actions). RIGHT: "The guard's eyes flick to the coin pouch at Kara's belt. What do you do?" (you set the scene and stopped). Exception: once a roll resolves a declared attempt, you DO narrate what the character did in that attempt; describing the declared lockpicking succeeding or failing from the dice is your job, not puppeting.
...
function describeSheet(sheet: CharacterSheet, playedBy: string, realDice: boolean): string {
  ...
  const lines = [
    ...
    `  HP ${sheet.currentHp}/${sheet.maxHp}${sheet.tempHp ? ` (+${sheet.tempHp} temp)` : ""} | AC ${sheet.ac} | Speed ${sheet.speed} | Passive Perception ${derived.passivePerception} | Initiative ${formatModifier(derived.initiative)} | Hit Dice ${Math.max(0, sheet.hitDice.total - sheet.hitDice.spent)}/${sheet.hitDice.total} ${sheet.hitDice.die}`,
    ...
    `  Features & traits (complete list; an ability not listed here does not exist for them): ${
      sheet.features?.length ? sheet.features.map((feature) => feature.name).join(", ") : "none"
    }${sheet.feats.length ? ` | Feats: ${sheet.feats.join(", ")}` : ""}`,
  ];
